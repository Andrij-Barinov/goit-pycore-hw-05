...
from log_analyzer import filter_logs_by_level, count_logs_by_level
from log_display import display_log_counts

def main():
    """
    The main function of the script. Processes command line arguments,
    loads logs, counts their number by level, and displays the results.
    If the logging level is specified, it also displays the details of the logs for this level.
    """
    if len(sys.argv) < 2:
    ...
    if log_level:
        # Filtering logs by the specified level
        filtered_logs = filter_logs_by_level(logs, log_level)
        if filtered_logs:
            print(f"\nLog details for the level '{log_level}':")
